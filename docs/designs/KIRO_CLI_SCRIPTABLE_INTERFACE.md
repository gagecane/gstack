...
- **New bead (optional):** "Migrate overlay-efficacy harness from systemPrompt append to Kiro modes" — philosophical, not blocking, may warrant its own design doc. **Resolved in [OVERLAY_EFFICACY_KIRO_MIGRATION.md](OVERLAY_EFFICACY_KIRO_MIGRATION.md)**: picked Path A (build full system prompt, feed via agent config) after discovering Kiro's "modes" are the agent list itself, not a distinct overlay layer. Path B's philosophical premise did not hold up on closer inspection. Port work is the follow-on bead.
